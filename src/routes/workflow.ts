...
export class Workflow extends Common {
  config() {
    ...
    router.get(
      ...
      awaitAppDelegateFactory(async (request, response) => {
        ...
        const instances = await bpmnAPI.data.findInstances(
          { "items.id": id },
          getSecureUser(request),
          "full"
        );
        ...
        let { node, fields } = await ViewHelper.getNodeInfo(
          bpmnServer,
          processName,
          elementId
        );
        ...
        try {
          let result = await bpmnAPI.engine.invoke(
            { "items.id": id },
            {},
            getSecureUser(request)
          );
    ...
    router.get(
      ...
      awaitAppDelegateFactory(async (request, response) => {
        ...
        let item = await bpmnAPI.data.findItem(
          { "items.id": id },
          getSecureUser(request)
        );
    ...
    router.post(
      ...
      awaitAppDelegateFactory(async (request, response) => {
        ...
        try {
          ...
          assignment["followUpDate"] = ViewHelper.dateInput(
            assignment["followUpDate"]
          );

          assignment["candidateUsers"] =
            assignment["candidateUsers"].split(",");
          assignment["candidateGroups"] =
            assignment["candidateGroups"].split(",");

          //console.log('data', data, 'assignment', assignment);
          let result = await bpmnAPI.engine.assign(
            { "items.id": id },
            data,
            assignment,
            getSecureUser(request)
    ...
    router.get(
      ...
      awaitAppDelegateFactory(async (request, response) => {
        ...
        let exec = await bpmnAPI.engine.start(
          process,
          { caseId: caseId++ },
          getSecureUser(request)
        );
  ...
  async tasks(request, response) {
    let items = await bpmnAPI.data.findItems(
      { "items.type": "bpmn:UserTask", "items.status": "wait" },
      getSecureUser(request)
    );
    ...
    let starts = await bpmnAPI.model.findStartEvents(
      {},
      getSecureUser(request)
    );
  ...
  if (process.env.REQUIRE_AUTHENTICATION === "true") {
    ...
    if (usr)
      user = new SecureUser({
        ...
        userGroups: usr.userGroups,
      });
  } else user = SecureUser.SystemUser();
...
function getUser(req) {
  if (req.session.forUser) return req.session.forUser;
...
function setForUser(req) {
  ...
  if ("forUserName" in req.query) {
    ...
    let userInfo = {
      ...
      userGroups: forUserGroups.split(","),
    };
...
async function display(req, res, title, output, logs = [], items = []) {
  let user = getSecureUser(req);
  var instances = await bpmnAPI.data.findInstances({}, user, {
    sort: { saved: -1 },
  });
  let waiting = await bpmnAPI.data.findItems(
  ...
  instances.forEach((item) => {
    item["fromNow"] = dateDiff(item.startedAt);
    if (item.endedAt) item["endFromNow"] = dateDiff(item.endedAt);
    else item["endFromNow"] = "";
  });
  ...
  res.render("index", {
    ...
    logs,
    items,
    forUserGroups: req.session.forUserGroups,
    forUserName: req.session.forUserName,
  });
}
...
function isAdmin(request) {
  console.log(
    process.env.REQUIRE_AUTHENTICATION,
    request.isAuthenticated(),
    request.user
  );

  if (process.env.REQUIRE_AUTHENTICATION !== "true") return true;
  else return request.isAdmin;
}
async function afterOperation(request, response, result) {
  ...
  } else display(request, response, "Show", []);
...
async function instanceDetails(request, response, instanceId, version) {
  ...
  let instance = await bpmnServer.dataStore.findInstance(
    { id: instanceId },
    "Full"
  );
